...
let conn = db.init();
router.post("/home/ex_1", function (req, res) {
...
router.post("/home/ex_2", function (req, res) {
  let user_keywords = req.body.find_keywords;
  ...
  conn.query(
    sql,
    [select_language, `%${user_keywords}%`, `%${user_keywords2}%`],
    function (err, rows) {
      if (!err) {
        if (rows.length > 0) {
          const randomIndex = Math.floor(Math.random() * rows.length);
...
router.post("/home/frontinput", function (req, res) {
...
router.post("/home/java_input", function (req, res) {
  let user_input = req.body.user_input; //사용자 입력 데이터 - X
  let select_language = req.body.select_language; //사용자 선택 언어 - O
  let f_text = req.body.f_text; // 문제   - O
...
router.post("/home/python_input", function (req, res) {
  let user_input = req.body.user_input; //사용자 입력 데이터 - X
  let select_language = req.body.select_language; //사용자 선택 언어 - O
...
router.post("/home/c_input", function (req, res) {
  let user_input = req.body.user_input; //사용자 입력 데이터 - X
  let select_language = req.body.select_language; //사용자 선택 언어 - O
